import { Link } from "@remix-run/react";

import NavDropdown from "./NavDropdown";
import NavLinkCustom from "./NavLinkCustom";

export default function Nav({ cartCount }) {
  return (
    <div id="nav" className="flex items-center bg-white">
      <img
        src="/img/alogo-4.png"
        alt="logo"
        className="max-h-24"
        // onMouseEnter={() => setShowSubmenu(false)}
      />
      <div className="flex absolute right-5 justify-between">
        <NavLinkCustom name="Home" route="/" />
        <NavLinkCustom name="Meet Our Team" route="/meet-our-team" />
        <NavDropdown
          name="Services"
          menu={[
            "Web Design & Development",
            "Hosting & Domains",
            "Site Migration",
            "Monthly Maintenance & Updates",
          ]}
        />
        <NavDropdown
          name="Tips & Tricks"
          menu={["Featured Articles", "Beginner", "Intermediate", "Advanced"]}
        />
        <NavLinkCustom name="Contact" route="/contact" />
        <NavDropdown
          name="Account"
          menu={["Account Settings", "Support", "License"]}
        />
        <Link to="/cart">
          <div className="flex">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-6 h-6"
              aria-label="cart"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z"
              />
            </svg>
            <div className="flex justify-center items-center px-2 rounded-full bg-blue-600 text-white  text-xs">
              {cartCount}
            </div>
          </div>
        </Link>
      </div>
    </div>
  );
}
